...
export default function ProjectCard({
  ...
  return (
    <motion.div
      ...
      aria-hidden="true"
      className="relative z-10 h-[550px] w-full items-stretch justify-center overflow-hidden rounded-3xl border border-gray-700 bg-zinc-200 dark:bg-zinc-800"
    >
      <motion.div
        ref={ref}
        animate={ctrls}
...
>
  {tech.map((item, index) => (
    <span
      key={index}
      className="rounded-md border border-foreground/30 px-3 py-1 text-xs font-semibold text-foreground/70 md:text-sm"
    >
      {item}
    </span>
  ))}
